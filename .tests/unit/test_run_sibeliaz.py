...
def test_run_sibeliaz(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/run_sibeliaz/config")
        data_path = Path(".tests/unit/run_sibeliaz/data")
        expected_path = Path(".tests/unit/run_sibeliaz/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/02_sibeliaz/blocks_coords.gff",
                "--snakefile",
                "Snakefile",
                "-f",
                "--notemp",
